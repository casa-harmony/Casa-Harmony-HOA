 ...
 * Demo transport.
 *
 * Nothing leaves the browser. `mockFetch` routes against the mutable store in
 ...
 *
 * This is one of two transports behind `lib/api.ts`; the other talks to the
 * FastAPI backend. Which one runs is decided by NEXT_PUBLIC_DATA_MODE.
 */

import { ApiError } from "../api-error";
import { ROLES } from "../rbac";
import { PERSONAS, TENANTS } from "./seed";
import { commit, nextId, nowIso, pushNotification, tenantData } from "./store";
...
export async function mockDownloadFile(
  path: string,
  _token: string,
  _tenantId: string,
  filename: string
