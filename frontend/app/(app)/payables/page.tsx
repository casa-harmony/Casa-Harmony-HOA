...
const TONE: Record<string, string> = {
  DRAFT: "none", SUBMITTED: "R", APPROVED: "O", ACCOUNTED: "A", REJECTED: "L", PAID: "A",
};
const MATCH_TONE: Record<string, string> = {
  NOT_MATCHED: "none", MATCHED: "A", MATCH_EXCEPTION: "L",
};
    ...
    <div className="space-y-5">
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="secondary" onClick={() => downloadFile(`/payables/register/export?start=${reg.start}&end=${reg.end}`, token!, activeTenantId!, "ap_invoice_register.xlsx")}>Invoice Register</Button>
          <Button variant="secondary" onClick={() => downloadFile(`/payables/distributions/export?start=${reg.start}&end=${reg.end}`, token!, activeTenantId!, "ap_distributions.xlsx")}>Distributions by Fund/Period</Button>
        </div>
      </Card>
      <Card>
        {loading ? (
              ...
              {invoices.map((inv) => (
                <tr key={inv.id} className="border-b border-slate-100">
                  <td className="py-2 pr-3 font-mono">
                    {inv.invoice_number}
                    {inv.on_hold && <span title={inv.hold_reason || ""}><Badge tone="L">HOLD</Badge></span>}
                  </td>
                  <td className="py-2 pr-3 text-right font-medium">${Number(inv.amount).toFixed(2)}</td>
                  <td className="py-2 pr-3 text-right text-slate-500">${Number(inv.tax_amount || 0).toFixed(2)}</td>
                  <td className="py-2 pr-3">
                    <Badge tone={MATCH_TONE[inv.match_status]}>{inv.match_status}</Badge>
                  </td>
                  <td className="py-2 pr-3"><Badge tone={TONE[inv.status]}>{inv.status}</Badge></td>
                  <td className="py-2 pr-3 text-right">
                    <div className="flex justify-end gap-2">
                      {inv.status === "DRAFT" && !inv.on_hold && (
                        <Button variant="secondary" onClick={() => submit(inv.id)} disabled={busy === inv.id}>Submit</Button>
                      )}
                      {!inv.on_hold && !["PAID", "CANCELLED"].includes(inv.status) && (
                        <Button variant="secondary" onClick={() => hold(inv.id)} disabled={busy === inv.id}>Hold</Button>
                      )}
                      {inv.on_hold && (
                        <Button variant="secondary" onClick={() => releaseHold(inv.id)} disabled={busy === inv.id}>Release</Button>
                      )}
                      {!["PAID", "CANCELLED"].includes(inv.status) && (
                        <Button variant="secondary" onClick={() => cancelInvoice(inv.id)} disabled={busy === inv.id}>Cancel</Button>
                      )}
                    </div>
                  </td>
                </tr>
